...
def login():
    ...
    if request.method == 'POST':
        ...
        if not conn:
            ...
            return render_template('login.html', masked_email=masked_email,
                                   last_login=last_login, last_device=last_device)
        try:
            ...
            if user and check_password_hash(user['password_hash'], password):
                ...
                try:
                    cursor.execute(
                        "INSERT INTO LoginActivity (customer_id, device_info, ip_address) VALUES (%s, %s, %s)",
...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO Customers (email, password_hash, full_name, phone) VALUES (%s, %s, %s, %s) RETURNING customer_id",
            ...
            cursor.execute(
                "INSERT INTO Accounts (customer_id, account_type, account_number, ifsc_code, branch_name, balance) VALUES (%s, %s, %s, %s, %s, %s)",
...
def dashboard():
    ...
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        query = """
            SELECT a.account_id, a.account_type, a.balance, a.account_number,
                   a.ifsc_code, a.branch_name, a.created_at, c.full_name, c.email
            FROM Accounts a
            INNER JOIN Customers c ON a.customer_id = c.customer_id
            WHERE c.customer_id = %s ORDER BY a.account_id
        """
        cursor.execute(query, (session['user_id'],))
        accounts = cursor.fetchall()
        cursor.execute(
            "SELECT login_timestamp, device_info FROM LoginActivity WHERE customer_id = %s ORDER BY login_timestamp DESC LIMIT 1",
            (session['user_id'],)
        )
        ...
        return render_template('dashboard.html', user_name=session['user_name'],
                               accounts=accounts, last_login=last_login)
    except Error as e:
...
def transfer():
    ...
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute(
            "SELECT account_id, account_type, balance, account_number, ifsc_code, branch_name FROM Accounts WHERE customer_id = %s ORDER BY account_id",
            (session['user_id'],)
        )
        user_accounts = cursor.fetchall()
        cursor.execute(
            "SELECT a.account_id, a.account_number, a.ifsc_code, a.branch_name, c.full_name FROM Accounts a JOIN Customers c ON a.customer_id = c.customer_id WHERE a.customer_id != %s ORDER BY c.full_name, a.account_id",
            (session['user_id'],)
        )
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                if not to_account:
                    flash(f'Account number {to_account_number} not found.', 'error')
                    return render_template('transfer.html', accounts=user_accounts, other_accounts=other_accounts)
            ...
            try:
                cursor.close()
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM TransferFunds(%s, %s, %s)", (from_account_id, to_account_id, amount))
...
def profile():
    ...
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute(
            "SELECT customer_id, full_name, email, phone, created_at FROM Customers WHERE customer_id = %s",
            (session['user_id'],),
        )
        customer = cursor.fetchone()
        cursor.execute(
            "SELECT account_id, account_type, account_number, ifsc_code, branch_name, balance, created_at FROM Accounts WHERE customer_id = %s ORDER BY account_id",
            (session['user_id'],),
        )
        accounts = cursor.fetchall()
        cursor.close()
        ...
        return render_template('profile.html', customer=customer, accounts=accounts, primary_account=primary_account)
    except Error as e:
...
def statement_pdf(account_id):
    ...
    try:
        ...
        # All transactions (no pagination for PDF)
        cursor.execute(
            "SELECT * FROM GetPaginatedStatement(%s, %s, %s)",
            (account_id, 1, 1000),
        )
        ...
        elements.append(HRFlowable(width='100%', thickness=1.2, color=colors.HexColor(blue)))
        ...
        elements.append(Paragraph(
            f'Transaction History  ({len(transactions)} records)', heading_style))
        ...
        if transactions:
            ...
            page_w = doc.width
            col_widths = [page_w*0.04, page_w*0.14, page_w*0.12,
                          page_w*0.28, page_w*0.10, page_w*0.18, page_w*0.14]
        ...
        elements.append(Paragraph(footer_logo,
            ParagraphStyle('Footer', parent=small_style,
...
def api_analytics():
    """
    GET /api/analytics?year=YYYY&month=MM&summary_year=YYYY
    Returns: { status, chart_data, credit_chart_data, monthly_totals,
               all_time_data, available_months, available_years,
               sel_year, sel_month, summary_year }
    """
    conn = get_db_connection()
    if not conn:
        return jsonify({"status": "error", "message": "Database error"}), 500
    ...
    try:
        ...
        if not user_accounts:
            ...
            return jsonify({
                "status": "success",
                "chart_data": [], "credit_chart_data": [],
                "monthly_totals": [], "all_time_data": [],
        ...
        # Debit category breakdown for selected month
        cursor.execute(f"""
            ...
            ORDER BY total DESC
        """, tuple(user_accounts) + (sel_year, sel_month))
        chart_data = [{"category": r['category'], "total": float(r['total']),
                       "txn_count": r['txn_count']} for r in cursor.fetchall()]

        # Credit category breakdown for selected month
        cursor.execute(f"""
            SELECT COALESCE(category, 'General') AS category,
                   SUM(amount) AS total, COUNT(*) AS txn_count
            FROM TransactionHistory
            ...
            ORDER BY total DESC
        """, tuple(user_accounts) + (sel_year, sel_month))
        credit_chart_data = [{"category": r['category'], "total": float(r['total']),
                               "txn_count": r['txn_count']} for r in cursor.fetchall()]
        ...
        cursor.execute(f"""
            ...
            ORDER BY DATE_TRUNC('month', transaction_timestamp) DESC
        """, tuple(user_accounts) * 4 + (summary_year,))
        monthly_totals = [{"month_label": r['month_label'],
        ...
        cursor.execute(f"""
            ...
            ORDER BY total DESC
        """, tuple(user_accounts) * 2)
        all_time_data = [{"category": r['category'], "total": float(r['total']),
                          "txn_count": r['txn_count']} for r in cursor.fetchall()]
...
def api_login():
    ...
    POST /api/login
    Body: email, password
    Returns: { status, message, user_name, user_email, user_id }
    """
    data = request.form
    ...
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute(
            "SELECT customer_id, email, password_hash, full_name FROM Customers WHERE email = %s",
            (email,)
        )
        user = cursor.fetchone()
...
def api_signup():
    """
    POST /api/signup
    Body: full_name, email, phone (optional), password, confirm_password
    Returns: { status, message, user_id, user_name, account_number }
    """
    data = request.form
    ...
    try:
        ...
        return jsonify({
            "status": "success",
            "message": f"Account created! Your account number is {account_number}",
            "user_id": customer_id,
            "user_name": full_name,
...
def api_dashboard():
    """
    GET /api/dashboard
    Returns: { status, user_name, last_login, accounts: [...] }
    """
    conn = get_db_connection()
    if not conn:
        return jsonify({"status": "error", "message": "Database error"}), 500
    ...
    try:
        ...
        cursor.execute(
            "SELECT login_timestamp FROM LoginActivity WHERE customer_id = %s ORDER BY login_timestamp DESC LIMIT 1",
            (session['user_id'],)
        )
...
def api_profile():
    """
    GET /api/profile
    Returns: { status, customer: {...}, accounts: [...] }
    """
    conn = get_db_connection()
    if not conn:
        return jsonify({"status": "error", "message": "Database error"}), 500
    ...
    try:
        ...
        customer = {
            ...
            "full_name": customer_raw['full_name'],
            "email": customer_raw['email'],
            "phone": customer_raw['phone'] or '',
            "created_at": str(customer_raw['created_at']),
        }
        accounts = [{
            "account_id": a['account_id'],
            "account_type": a['account_type'],
...
def api_transfer_get():
    """
    GET /api/transfer
    Returns: { status, user_accounts: [...], other_accounts: [...] }
    """
    conn = get_db_connection()
    if not conn:
        return jsonify({"status": "error", "message": "Database error"}), 500
    ...
    try:
        ...
        cursor.execute(
            "SELECT a.account_id, a.account_number, a.ifsc_code, a.branch_name, c.full_name FROM Accounts a JOIN Customers c ON a.customer_id = c.customer_id WHERE a.customer_id != %s ORDER BY c.full_name",
            (session['user_id'],)
        )
...
def api_transfer_post():
    """
    POST /api/transfer
    Body: from_account, to_account (12-digit number), amount, beneficiary_name, bank_name, ifsc
...
def api_statement(account_id):
    """
    GET /api/statement/<account_id>?page=1
    Returns: { status, account: {...}, transactions: [...], page, total_pages, total_records }
    """
    page = request.args.get('page', 1, type=int)
    if page < 1:
        page = 1
    ...
    try:
        ...
        cursor.execute(
            "SELECT account_id, account_type, balance, account_number, ifsc_code FROM Accounts WHERE account_id = %s",
            (account_id,),
        )
        ...
        account = {
            ...
            "account_type": account_raw['account_type'],
            "balance": float(account_raw['balance']),
            "account_number": account_raw['account_number'],
            "ifsc_code": account_raw['ifsc_code'],
        }
